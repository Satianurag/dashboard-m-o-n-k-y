...
} from '@/shared/types/pnode';
import { getClusterNodes } from '@/features/pnodes';
import { getNetworkStats, getGossipHealth } from '@/features/network';
import { ASNS, hashPubkey, getTier } from '@/features/pnodes/services/utils';
...
export async function getTrendData(metric: string, period: '24h' | '7d' | '30d' = '24h'): Promise<TrendData> {
    const history = await import('@/features/network').then(m => m.getPerformanceHistory(period));
